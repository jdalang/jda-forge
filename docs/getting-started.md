...
| Position | File(s) | Why |
|---|---|---|
| First | `_build/assets.jda` | Defines `forge_stylesheet_tag`, `forge_javascript_tag` — used by the layout |
| Second | `config/application.jda` | Defines `load_env`, `app_config`, and constants everything else uses |
| Third | `app/helpers/*.jda` | Defines `h()`, `link_to()`, `pluralize()` that views and controllers call |
| Fourth | `_build/models.jda` | CRUD functions + typed row structs auto-generated from migrations |
| Fifth | `_build/views.jda` | Compiled ERB templates — view functions that controllers call |
| Sixth | `app/models/*.jda` | Model init — associations, callbacks, validations |
| Seventh | `app/controllers/*.jda` | Action functions registered as route handlers |
| Eighth | `_build/routes.jda` | Path helpers + `routes(app)` compiled from `config/routes.jda` |
