...
| # | Issue | Per-request cost | Fix complexity |
|---|-------|-----------------|----------------|
| 1 | New DB connection per query | 40–240ms | Low — persistent global fd |
| 2 | Single-threaded server | 0 concurrency | Medium — compiler `spawn` + pthread |
| 3 | mmap per string alloc | ~50µs, memory waste | Medium — arena allocator |
| 4 | No HTTP keep-alive | TCP overhead per request | Low — loop in forge_handle_fd |
| 5 | 2MB alloc per query | Memory pressure | Low — reduce alloc_pages(512) |
